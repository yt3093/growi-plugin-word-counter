...
### 9. SVG `<text>` の垂直中央揃えは `dominant-baseline="central"` より `"middle"` が安定する

`ICON_CHARS`（円バッジ内の「A」1文字）で `dominant-baseline: central` を使うと、円の中心からわずかに下寄りに表示されるブラウザ挙動があった（`central` は降下部（ディセンダー）を持つ文字も含めた汎用的な基準線で、"A" のようにディセンダーの無い文字だけを厳密に中央揃えする用途には合わない）。`dominant-baseline: middle` + `y` を中心座標そのもの（このアイコンでは `y="12"`、`cx`/`cy` と同じ値）に設定する組み合わせの方が、単一文字を円や図形の中心に揃える用途では安定する。
